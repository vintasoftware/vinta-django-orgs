...
from organizations.helpers.organizations import create_organization, set_current_organization
...
class PermissionQueryCountTests(OrganizationsTestCase):
    def test_repeated_checks_are_free(self) -> None:
        self.user.has_perm('organizations.add_organization')

        with self.assertNumQueries(0):
            self.user.has_perm('organizations.change_organization')
            self.user.has_perm('organizations.delete_organization')

    def test_a_permission_the_user_lacks_is_cached_too(self) -> None:
        # An empty set is an answer, not a miss.
        self.user.has_perm('organizations.nonexistent_permission')
