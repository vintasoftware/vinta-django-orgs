...
class OrganizationMiddlewareTests(TestCase):
    @mock.patch('vinta_orgs.middleware.OrganizationMiddleware.process_request')
    @mock.patch('vinta_orgs.middleware.OrganizationMiddleware.process_response')
    def test_calls_process_request_and_process_response(
        ...
        request = factory.get(reverse('vinta_orgs:organization_list'), HTTP_HOST='test.localhost:8000')
    ...
    @mock.patch('vinta_orgs.middleware.get_organization')
...
class GetOrganizationTests(TestCase):
    ...
    def test_with_http_header(self) -> None:
        organization = create_organization(name='test', slug='test', domains=['test.localhost:8000'])
        factory = RequestFactory()
        request = factory.get(reverse('vinta_orgs:organization_list'), HTTP_ORGANIZATION_SLUG=organization.slug)
    ...
    def test_with_unexistent_organization_in_http_header(self) -> None:
        create_organization(name='test', slug='test', domains=['test.localhost:8000'])
        factory = RequestFactory()
        request = factory.get(reverse('vinta_orgs:organization_list'), HTTP_ORGANIZATION_SLUG='unexistent')
    ...
    def test_with_previously_set_organization(self) -> None:
        organization = create_organization(name='test', slug='test', domains=['test.localhost:8000'])
        factory = RequestFactory()
        request = factory.get(reverse('vinta_orgs:organization_list'))
