...
from organizations_custom_data.serializers import OrganizationSpecificFieldDefinitionCreateSerializer
...
class CustomTablesListTests(OrganizationsAPITestCase):
    def setUp(self) -> None:
        ...
        self.view_url = reverse('organizations_custom_data:custom_tables_list')
        validator_gt_2 = baker.make(
            OrganizationSpecificFieldsValidator,
            module_path='organizations_custom_data.tests.validators.validator_gt_2',
        )
...
class CustomTablesDetailsTests(OrganizationsAPITestCase):
    def setUp(self) -> None:
        ...
        self.custom_table_view_url = reverse(
            'organizations_custom_data:custom_tables_details',
            kwargs={'slug': '_custom_tables__' + self.tables[0].name},
        )
        self.customizable_model_view_url = reverse(
            'organizations_custom_data:custom_tables_details', kwargs={'slug': 'lectures__lecture'}
        )
...
class OrganizationSpecificTableRowViewsetTests(OrganizationsAPITestCase):
    def setUp(self) -> None:
        ...
        self.list_view_url = reverse(
            'organizations_custom_data:custom_data_list',
            kwargs={
                'slug': '_custom_tables__' + self.table.name,
            },
        )
        self.details_view_url = reverse(
            'organizations_custom_data:custom_data_details',
            kwargs={
                'slug': '_custom_tables__' + self.table.name,
                'pk': self.row.id,
            },
        ...
        for i, field in enumerate(self.fields):
            if i == 0:
                validator_lt_2 = baker.make(
                    OrganizationSpecificFieldsValidator,
                    module_path='organizations_custom_data.tests.validators.validator_lt_2',
                )
